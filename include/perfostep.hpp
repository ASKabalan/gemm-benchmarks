...
public:
  ...
  virtual void Report(const bool &iPrintTotal) const = 0;
  virtual void UpdateCols(const ColumnNames &iCol) = 0;
  virtual void PrintToMarkdown(const std::string &ifilename,
                               const bool &iPrintTotal) const = 0;
  virtual void PrintToCSV(const std::string &ifilename,
                          const bool &iPrintTotal) const = 0;
...
public:
  void Report(const bool &iPrintTotal) const override {
    if (m_Reports.size() == 0)
      return;
  ...
  void UpdateCols(const ColumnNames &iCol) override { m_ColNames = iCol; }

  void PrintToMarkdown(const std::string &filename,
                       const bool &iPrintTotal) const override {
    if (m_Reports.size() == 0 || filename.empty())
      return;
...
public:
  ...
  void Start(const std::string &iReport, const ColumnNames &iCol = {}) {
    std::cout << "Starting " << iReport << std::endl;
  ...
  void UpdateCols(const ColumnNames &iCol) {
    if (m_EnablePerfoStep)
      m_Perfostep->UpdateCols(iCol);
  }

  void Report(const bool &iPrintTotal = false) {
    if (m_EnablePerfoStep)
  ...
  void PrintToCSV(const std::string &filename, const bool &iPrintTotal = false) const {
    if (m_EnablePerfoStep)
